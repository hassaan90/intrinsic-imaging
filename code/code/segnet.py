...
def downsampling_block_basic(inputs, n_filters, filter_size,
    ...
    conv = Convolution2D(n_filters, filter_size, filter_size,
                         border_mode='same', init= 'he_normal',W_regularizer=W_regularizer)(inputs)
...
def upsampling_block_basic(inputs, n_filters, filter_size, unpool_layer=None,
    ...
    conv = Convolution2D(n_filters, filter_size, filter_size,init='he_normal',
                         border_mode='same', W_regularizer=W_regularizer)(up)
    bn = BatchNormalization(mode=0, axis=channel_idx())(conv)
    #bn = Dropout(0.5)(bn)
    act = Activation('relu')(bn)
    return act
def direct_intrinsic(img_shape=(3, None, None) ,weight_path='', activation='relu', init='he_normal'):

    #act = keras.layers.advanced_activations.PReLU(init='zero', weights=None)

    act = activation
    inputs = Input(img_shape)
    conv1 = Convolution2D(96, 11, 11, subsample=(4, 4), activation = act,  init = init, border_mode='same')(inputs)
    #act1 = Activation(PReLU(init='zero', weights=None))(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)

    conv2 = Convolution2D(256, 5, 5, activation=act,  init = init, border_mode='same')(pool1)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)

    conv3 = Convolution2D(256, 3, 3, activation=act, init = init, border_mode='same')(pool2)
    #pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)

    conv4 = Convolution2D(386, 3, 3, subsample=(2, 2), activation=act,init = init, border_mode='same')(conv3)
    #pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
    #pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)

    conv5 = Convolution2D(256, 3, 3, activation=act, init = init, border_mode='same')(conv4)
    #conv5 = Convolution2D(512, 3, 3, activation='relu', border_mode='same')(conv5)
    up = UpSampling2D(size= (8, 8))(conv5)
    #conv6 = Convolution2D(256, 3, 3, activation=act,init = init, border_mode='same')(up)
    conv7 = Convolution2D(64, 1, 1, activation=act, init = init, border_mode='same')(up)
    
    conv2_1 = Convolution2D(96, 9, 9, activation=act, subsample=(2, 2), init = init, border_mode='same')(inputs)
    pool2_1 = MaxPooling2D(pool_size=(2, 2))(conv2_1)
    merge1 = merge((pool2_1, conv7), mode='concat', concat_axis=1)
    conv2_2 = Convolution2D(64, 5, 5, activation=act,init = init,  border_mode='same')(merge1)
    conv2_3 = Convolution2D(64, 5, 5, activation=act, init = init, border_mode='same')(conv2_2)
    conv2_4 = Convolution2D(64, 5, 5, activation=act, init = init, border_mode='same')(conv2_3)
    conv2_5_1 = Convolution2D(3, 5, 5, activation=act,  init = init, border_mode='same')(conv2_4)
    deconv2_5_1 = Deconvolution2D(3, 8, 8, subsample=(4, 4), activation= act, init= init, border_mode = 'same', name = 'albedo', output_shape=(None, 3, img_shape[1], img_shape[2]))(conv2_5_1) 
    conv2_5_2 = Convolution2D(3, 5, 5, activation=act,  init = init, border_mode='same')(conv2_4)
    deconv2_5_2 = Deconvolution2D(3, 8, 8, subsample=(4, 4), activation= act, init= init, border_mode = 'same', name = 'shading', output_shape=(None, 3, img_shape[1], img_shape[2]))(conv2_5_2) 
    model = Model(input=inputs, output=[deconv2_5_1, deconv2_5_2])
    if weight_path <> '':
...
def build_segnet_basic(inputs, n_classes, depths=[64, 128, 256, 512, 512],
    ...
    """ logits """
    conv10 = Convolution2D(n_classes, 1, 1, border_mode='valid', init='he_normal', activation='sigmoid')(dec5)
...
def build_segnet(img_shape=(3, None, None), n_classes=8, l2_reg=0.,
    ...
    # Create basic Segnet
    if basic:
        model = build_segnet_basic(inputs, n_classes, [64, 128, 256, 512, 512],
                                   7, l2_reg)
    else:
...
#    model_basic = build_segnet(img_shape=(3, 160, 224), n_classes=3, l2_reg=0.,
#                 init='he_normal', path_weights=None,
#                 freeze_layers_from=None, use_unpool=False, basic=True)
#    print ('COMPILING basic segnet')
#    model_basic.compile(loss="mse", optimizer="adam", metrics=['accuracy'])
#    model_basic.summary()
#    print ('END COMPILING basic segnet')
#    print model_basic.summary
#    plot(model_basic, to_file='model.png', show_shapes = True, show_layer_names = True)
    
    
    model_direct = direct_intrinsic(img_shape=(3, 64, 64), weight_path='', activation= 'relu', init = 'he_normal' )
    model_direct.compile(loss={'albedo': 'mse', 'shading': 'mse'}, optimizer="adam", metrics=['accuracy'])
    model_direct.summary()
    plot(model_direct, to_file='model.png', show_shapes = True, show_layer_names = True)
